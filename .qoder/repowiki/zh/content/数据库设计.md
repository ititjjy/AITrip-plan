...
- Agent本地数据库位于 agent/db.ts，负责采集数据缓存、采集日志、采集批次、刷新周期、原始采集数据、待确认更新、城市统计等。
...
A_BATCHES["collection_batches 表"]
A_RAW["raw_pois 表"]
...
- Agent本地数据库（agent.db）
  ...
  - 采集日志 collection_logs：采集来源、状态、耗时、错误信息、按类别统计。
  - 采集批次 collection_batches：批次类型、状态、开始/完成时间、城市数量、配置与结果。
...
- 新增的采集批次表 collection_batches 支持批量采集任务的跟踪与管理。
...
图表来源
- [server/db.ts:46-144](file://server/db.ts#L46-L144)
- [agent/db.ts:94-100](file://agent/db.ts#L94-L100)
- [agent/db.ts:186-196](file://agent/db.ts#L186-L196)
...
- 初始化与表结构
  - 启用WAL与外键检查。
  - 定义 city_pois、collection_logs、collection_batches、raw_pois、pending_updates、city_stats、refresh_cycles 等表。
- 关系与约束
  - raw_pois 主键为 (city_id, source)，确保每城市每来源仅保留最新一次采集。
  - city_pois.version 作为安全的版本号字段，支持增量更新与一致性判断。
- 新增采集批次管理
  - collection_batches：支持批次类型（如批量采集、增量更新）、状态跟踪（运行中/完成/失败）、开始/完成时间、城市数量、配置与结果存储。
  - 通过 getCollectionBatches/getCollectionBatch/updateCollectionBatch 等函数管理批次生命周期。
- 增强采集日志统计
  - collection_logs：新增 by_category 字段，支持按POI类别统计采集结果，使用JSON格式存储类别计数。
  - 通过 ALTER TABLE 语句安全添加 by_category 列，初始默认值为空字典。
  - 日志记录包含 items_collected/items_accepted 计数器，支持更精细的质量统计。
- 采集与统计
  - upsertPOIs：按城市Upsert POI集合。
  - logCollection：记录采集日志（来源、状态、数量、耗时、错误、按类别统计）。
...
- 索引策略
  ...
  - collection_batches 上建立 created_at 索引，支撑批次查询与排序。
  - raw_pois 上建立 city_id 索引，支撑按城市检索。

章节来源
- [agent/db.ts:34-131](file://agent/db.ts#L34-L131)
- [agent/db.ts:86-100](file://agent/db.ts#L86-L100)
- [agent/db.ts:94-100](file://agent/db.ts#L94-L100)
- [agent/db.ts:186-196](file://agent/db.ts#L186-L196)
- [agent/db.ts:484-551](file://agent/db.ts#L484-L551)
...
### 数据模型设计原理
- 字段定义与数据类型
  - 文本型：邮箱、来源名、城市ID、标题、封面图、内容、状态、类型、配置JSON、结果JSON。
  - 整型：ID、计数、布尔模拟（0/1）、时间戳（毫秒）、城市数量。
  - 浮点型：价格、经纬度、质量分数。
  - JSON字符串：trip_data、data字段、config、results、by_category用于存储结构化数据，便于灵活扩展。
- 主外键约束
...
- 业务规则
  ...
  - collection_batches 支持批次状态机管理，自动记录完成时间。
  - collection_logs.by_category 提供按类别统计的结构化数据。
...
- Agent初始化
  ...
  - 自动创建 agent.db 与所有表。
  - 安全添加 collection_logs.by_category 列，使用 ALTER TABLE 语句。
- 迁移策略（服务端 city_pois）
  - 检测旧表是否包含 season 列。
...
- 索引策略
  ...
  - collection_batches(idx_batches_created)：加速批次查询与排序。
  - raw_pois(idx_raw_city)：加速按城市检索原始采集数据。
- JSON字段访问
  - 使用JSON存储结构化数据，避免复杂JOIN；必要时在应用层进行JSON解析与聚合。
  - collection_logs.by_category 支持JSON聚合查询，但需注意索引限制。
- 批量操作
...
- 维护
  ...
  - collection_batches 需要定期清理已完成的批次记录以控制表规模。
...
- 组件耦合
  ...
  - 新增的 collection_batches 与 collection_logs 通过 city_id 关联，支持批次级别的日志追踪。
- 外部依赖
...
- 配置影响
  ...
  - admin/types/index.ts 中的类型定义影响前端对 collection_batches 和 collection_logs 的显示。
...
AdminTypes["admin/types/index.ts<br/>CollectionBatchInfo, CollectionLogEntry"] --> AdminUI["admin/pages/CollectionDetail.tsx"]
AgentDB --> |读写| AgentDB
...
- [admin/types/index.ts:309-343](file://admin/types/index.ts#L309-L343)
...
- 索引与扫描
  - 为高频查询字段建立索引（如 collection_logs 的复合索引、collection_batches 的时间索引、raw_pois 的城市索引）。
- 事务与批量
  - 将相关写操作放入单个事务，减少提交次数。
- 缓存命中
...
- 清理策略
  ...
  - collection_batches 需要定期归档或清理已完成批次。

## 故障排查指南
- 表结构异常
  - 使用 PRAGMA table_info(table) 检查列定义，结合迁移脚本逻辑定位差异。
  - 特别关注 collection_logs.by_category 列的存在性。
- 外键约束错误
...
- 采集日志异常
  ...
  - 验证 by_category 字段的JSON格式是否正确。
- 采集批次异常
  - 检查 collection_batches 的状态流转是否符合预期。
  - 验证 started_at/completed_at 时间戳的一致性。
- 验证码无效
...
## 结论
本设计以SQLite为核心，服务端与Agent侧分别承担用户/旅行计划与采集/缓存职责，通过WAL与外键保障一致性，借助索引与缓存提升性能。最新的变更引入了 collection_batches 表支持批次管理，增强了 collection_logs 表的按类别统计能力，使采集流程更加规范化和可观测。迁移脚本确保表结构随业务演进而平滑升级，初始化与维护流程清晰可操作。遵循本文的查询优化、缓存策略与运维最佳实践，可在保证数据一致性的前提下获得稳定高效的运行表现。
...
- 实际SQL示例路径（仅提供路径，不展示具体代码）
  ...
  - 添加 collection_logs.by_category 列：[agent/db.ts:86-100](file://agent/db.ts#L86-L100)
  - 创建 collection_batches 表：[agent/db.ts:94-100](file://agent/db.ts#L94-L100)
  - 服务端POI缓存Upsert与查询：[server/db.ts:253-261](file://server/db.ts#L253-L261)
  - Agent采集日志记录（含类别统计）：[agent/db.ts:186-196](file://agent/db.ts#L186-L196)
  - Agent批次管理操作：[agent/db.ts:484-551](file://agent/db.ts#L484-L551)
  ...
  - 管理端采集批次详情显示：[admin/types/index.ts:309-343](file://admin/types/index.ts#L309-L343)