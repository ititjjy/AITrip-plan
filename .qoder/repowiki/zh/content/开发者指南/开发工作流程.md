...
### 路线规划算法组件分析
路线规划算法基于地理距离与时间窗口，实现智能行程生成，包括POI分类、地理聚类、贪心路径搜索、2-opt优化、餐食插入与自动填充等。

```mermaid
flowchart TD
Input["输入: 选中POI, 天数计划, 城市ID"] --> Classify["POI分类(餐饮/非餐饮)"]
Classify --> Cluster["地理聚类分配到各天"]
Cluster --> Route["贪心路径+2-opt优化"]
Route --> Schedule["时间调度(开放时间约束)"]
Schedule --> Meals["餐食插入(地理+时间评分)"]
Meals --> AutoFill["自动填充空隙POI"]
AutoFill --> Output["输出: 日程安排"]
```

**图表来源**
- [src/utils/routePlanner.ts:672-680](file://src/utils/routePlanner.ts#L672-L680)
