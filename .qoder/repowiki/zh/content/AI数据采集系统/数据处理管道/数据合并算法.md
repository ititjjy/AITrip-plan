...
- [scripts/enrich-hotel-fields.ts](file://scripts/enrich-hotel-fields.ts)
- [scripts/enrich-district-amap.ts](file://scripts/enrich-district-amap.ts)
- [server/dedup.ts](file://server/dedup.ts)
</cite>

## 更新摘要
**所做更改**
- 新增酒店特定字段处理：distanceFromCenter 和 district 字段的自动计算与提取
- 城市坐标直接计算距离：利用 city-coords.json 中的城市中心坐标进行精确距离计算
- 批量酒店字段增强：通过独立脚本为现有酒店数据补充缺失的距离和行政区字段
- 地址行政区提取优化：改进的 extractDistrict 函数支持国内外地址解析
...
- **新增** 酒店特定字段处理：distanceFromCenter（距离市中心直线距离）和 district（行政区/县名）的自动计算与提取。
- **新增** 城市坐标直接计算距离：利用 city-coords.json 中的城市中心坐标进行精确距离计算。
- 完整数据合并流程：输入数据格式、处理步骤与输出结果。
- 复杂度分析与性能调优建议。

## 项目结构
本项目采用模块化设计，核心算法集中在 agent 目录，数据模型与类别体系在 sources/base.ts 与 categories.ts 中定义，相似度与分类器在 similarity.ts 与 classifier.ts 中实现，工具函数在 utils.ts 中提供距离与并发控制等支持。新增的酒店字段处理通过独立脚本实现，与主合并流程解耦。
...
CONF["agent/config.ts<br/>配置管理"]
COOR["agent/data/city-coords.json<br/>城市坐标数据"]
end
subgraph "酒店字段增强"
EH["scripts/enrich-hotel-fields.ts<br/>批量酒店字段补充"]
EDA["scripts/enrich-district-amap.ts<br/>高德API行政区提取"]
end
subgraph "Server 对照"
...
**图表来源**
- [agent/merger.ts:1-1170](file://agent/merger.ts#L1-L1170)
...
- [agent/sources/base.ts:1-257](file://agent/sources/base.ts#L1-L257)
- [agent/config.ts:1-182](file://agent/config.ts#L1-L182)
- [agent/data/city-coords.json:1-2378](file://agent/data/city-coords.json#L1-L2378)
- [scripts/enrich-hotel-fields.ts:1-116](file://scripts/enrich-hotel-fields.ts#L1-L116)
- [scripts/enrich-district-amap.ts:35-117](file://scripts/enrich-district-amap.ts#L35-L117)
...
- **新增** 酒店特定字段处理：distanceFromCenter 和 district 字段的自动计算与提取。
- **新增** 城市坐标集成：通过 city-coords.json 提供精确的城市中心坐标用于距离计算。
...
- [agent/merger.ts:929-933](file://agent/merger.ts#L929-L933)
...
- [agent/sources/base.ts:178-181](file://agent/sources/base.ts#L178-L181)
...
- **新增** 酒店字段增强：为酒店类目自动计算距离市中心距离和提取行政区信息。
...
#### 距离市中心计算
- **城市坐标集成**：通过 agent/data/city-coords.json 提供精确的城市中心坐标（lat、lng）。
- **实时距离计算**：在 rawToPOI 函数中，对酒店类目（categoryL1='hotel'）自动计算距离市中心的直线距离。
- **精度控制**：距离结果保留 0.1km 精度，使用 haversineDistance 函数进行精确计算。

#### 行政区/县名提取
- **地址解析策略**：extractDistrict 函数支持国内外地址的智能解析。
- **国内地址**：优先匹配"XX区"或"XX县"（排除省、市），适用于中国地址。
- **国外地址**：支持英文地址中的常见行政区模式，如"Xxx District"、"Xxx Regency"等。
- **回退机制**：当无法匹配特定模式时，尝试解析逗号分隔地址的倒数第二个字段。

```mermaid
flowchart TD
HotelPOI["酒店 POI"] --> CheckL1{"categoryL1 === 'hotel'?"}
CheckL1 --> |是| CalcDist["计算距离市中心"]
CalcDist --> ExtractDist["提取行政区名"]
ExtractDist --> SetFields["设置 distanceFromCenter & district"]
CheckL1 --> |否| Skip["跳过"]
SetFields --> Output["输出 POI"]
Skip --> Output
```

**图表来源**
- [agent/merger.ts:929-933](file://agent/merger.ts#L929-L933)
- [agent/merger.ts:939-966](file://agent/merger.ts#L939-L966)
...
#### enrich-hotel-fields.ts
- **批量处理**：扫描 agent.db 中的所有城市数据，为酒店 POI 补充缺失的字段。
- **距离计算**：使用城市中心坐标计算每个酒店到市中心的距离。
- **行政区提取**：从地址中提取行政区/县名，支持中英文地址。
- **数据库更新**：将计算结果写回 agent.db 的 city_pois 表。

#### enrich-district-amap.ts  
- **API 集成**：通过高德地图 API 获取缺失的行政区信息。
- **批量处理**：按城市分批处理，最小化 API 调用频率。
- **错误处理**：完善的异常处理和重试机制，确保数据完整性。
...
- 处理步骤：
  ...
  7) **新增** 酒店字段增强：自动计算距离市中心距离和提取行政区信息。
  8) 冲突检测与评分：生成冲突报告与 POIScore。
  9) 后分类检查：最终校验与评分。
...
**章节来源**
- [agent/merger.ts:492-1170](file://agent/merger.ts#L492-L1170)
- [agent/sources/base.ts:121-182](file://agent/sources/base.ts#L121-L182)
...
- **新增** city-coords.json 提供城市中心坐标数据，被 merger.ts 和独立脚本共同使用。
- **新增** scripts/enrich-hotel-fields.ts 和 scripts/enrich-district-amap.ts 作为独立的酒店字段增强工具。
...
M --> CONF["config.ts"]
M --> COOR["city-coords.json"]
EH["enrich-hotel-fields.ts"] --> COOR
EDA["enrich-district-amap.ts"] --> COOR
S --> U
C --> CAT
...
- [agent/config.ts:155-160](file://agent/config.ts#L155-L160)
- [server/dedup.ts:1-25](file://server/dedup.ts#L1-25)
...
- 时间复杂度
  ...
  - **新增** 酒店字段计算：O(h) 对于 h 个酒店 POI，每次计算涉及一次 haversine 距离计算和地址解析。
- 空间复杂度
  - 并查集：O(n)。
  - 地理桶：O(n)。
  - 冲突检测：O(g²)。
  - **新增** 城市坐标缓存：O(c) 对于 c 个城市。
- 优化建议
  ...
  - **新增** 缓存策略：city-coords.json 可以缓存在内存中，避免重复读取文件。
  - **新增** 批量处理：酒店字段增强脚本采用批量处理方式，减少数据库频繁访问。
...
- **新增** 酒店字段计算异常
  - 检查 city-coords.json 中是否存在对应城市的坐标数据。
  - 验证 extractDistrict 函数的地址解析逻辑，确认地址格式是否符合预期。
  - 参考：[agent/merger.ts:929-933](file://agent/merger.ts#L929-L933)、[agent/merger.ts:939-966](file://agent/merger.ts#L939-L966)
- **新增** 批量酒店字段增强失败
  - 检查 agent.db 连接状态和 city_pois 表结构。
  - 验证高德 API 密钥配置（如使用 enrich-district-amap.ts）。
  - 参考：[scripts/enrich-hotel-fields.ts:55-116](file://scripts/enrich-hotel-fields.ts#L55-L116)、[scripts/enrich-district-amap.ts:63-117](file://scripts/enrich-district-amap.ts#L63-L117)
...
- [agent/merger.ts:929-933](file://agent/merger.ts#L929-L933)
- [agent/merger.ts:939-966](file://agent/merger.ts#L939-L966)
- [scripts/enrich-hotel-fields.ts:55-116](file://scripts/enrich-hotel-fields.ts#L55-L116)
- [scripts/enrich-district-amap.ts:63-117](file://scripts/enrich-district-amap.ts#L63-L117)

## 结论
本数据合并算法通过 5路径决策树实现精准的相似度评估，结合地理预分桶与 Union-Find 传递性去重，有效降低比较复杂度；通过冲突检测与评分体系保证合并结果的质量；跨类目去重机制进一步提升覆盖率与准确性。**新增的酒店特定字段处理功能**通过城市坐标直接计算距离和智能地址解析，显著提升了酒店数据的实用性和准确性。整体流程清晰、模块化程度高，便于扩展与维护。

## 附录
- 数据模型字段说明
  - RawPOI：主名、中文名、英文名、L1/L3 类目、坐标、地址、评分、费用、时长、描述、标签、营业时间、最佳季节、月度指数、来源等。
  - POI：与 RawPOI 类似，但字段标准化、补充图片 URL、体验类目专属字段与质量评分。**新增** distanceFromCenter（距离市中心直线距离）和 district（行政区/县名）字段。
- 类目体系
  - L1 类别：scenic、food、shopping、entertainment、experience、hotel。
  - L3 类目：由 L1 与 L2 组合构成，支持路径解析与映射。
- **新增** 城市坐标数据
  - city-coords.json：包含城市中心坐标（lat、lng）、国家、洲、省份等地理信息，用于酒店距离计算和行政区提取。
