...
```mermaid
flowchart TD
Start(["开始"]) --> Collect["采集 RawPOI"]
Collect --> Validate["无效数据过滤"]
Validate --> GeoBucket["地理分桶优化"]
GeoBucket --> Similarity["复合相似度计算"]
Similarity --> UF["Union-Find 去重"]
UF --> CrossCat["跨类目去重"]
CrossCat --> Merge["数据合并加权/中位数/最长"]
Merge --> Translate["名称翻译补齐"]
Translate --> Score["质量评分与冲突检测"]
Score --> Save["写入数据库"]
Save --> End(["结束"])
```
