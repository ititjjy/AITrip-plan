...
## 简介
本文件面向前端开发者，系统性梳理客户端可直接使用的API端点与交互规范，覆盖POI查询、旅行规划、用户管理、地图服务、酒店与预订、游记与评论、**城市数据管理**等核心能力。文档同时说明HTTP方法、URL模式、请求参数、响应格式、错误码、鉴权方式、版本控制策略与向后兼容性，并提供客户端集成要点、SDK使用建议、性能优化与调试技巧。
...
- **新增**：城市数据管理功能位于 server/admin-routes.ts，提供城市注册表的增删改查操作。
...
ADMIN["管理端<br/>admin/pages/Cities.tsx"] --> ADMIN_API["管理端API<br/>admin/lib/api.ts"]
ADMIN_API --> ADMIN_ROUTES["管理端路由<br/>server/admin-routes.ts"]
ADMIN_ROUTES --> CITY_REGISTRY["城市注册表<br/>scripts/city-registry.json"]
```
...
- [server/admin-routes.ts:564-672](file://server/admin-routes.ts#L564-L672)
...
- **新增**：城市数据管理：提供城市注册表的查询、添加、修改、删除功能，支持移动应用的动态城市数据支持。
...
- [server/index.ts:109-118](file://server/index.ts#L109-L118)
- [server/admin-routes.ts:564-672](file://server/admin-routes.ts#L564-L672)

## 架构总览
后端采用Express + SQLite方案，提供统一REST API；前端通过fetch调用后端接口，部分流程包含轮询与缓存策略，确保用户体验与性能平衡。**新增**的城市数据管理功能通过独立的管理端路由实现，支持对城市注册表的实时维护。

```mermaid
sequenceDiagram
participant C as "客户端"
participant S as "后端API(server/index.ts)"
participant A as "管理端API(server/admin-routes.ts)"
participant D as "数据库(server/db.ts)"
C->>S : "GET /api/cities"
S->>D : "读取城市注册表"
D-->>S : "返回城市列表"
S-->>C : "返回成功响应"
C->>A : "POST /api/admin/cities"
A->>D : "写入城市注册表"
D-->>A : "确认写入"
A-->>C : "返回新增城市信息"
```
...
**图表来源**
- [server/index.ts:109-118](file://server/index.ts#L109-L118)
- [server/admin-routes.ts:564-591](file://server/admin-routes.ts#L564-L591)
- [server/db.ts:1-200](file://server/db.ts#L1-L200)
...
- 端点
  - GET /api/cities
    - 功能：获取所有城市注册表数据
    - 返回字段：success、data（城市数组）
  - POST /api/admin/cities
    - 功能：添加新城市到注册表
    - 请求体：id、name、nameEn、continent、country、province、lat、lng
    - 返回字段：success、data（新增城市信息）
  - PUT /api/admin/cities/:id
    - 功能：更新现有城市信息
    - 参数：id（路径参数）
    - 请求体：name、nameEn、continent、country、province、lat、lng
    - 返回字段：success
  - DELETE /api/admin/cities/:id
    ...
    - 返回字段：success
- 数据结构
  - 城市对象：id、name、nameEn、continent、country、province、lat、lng、poiCount、lastUpdated
  - 城市注册表：包含城市ID、名称、英文名、热度等级等信息
- 错误码
  - MISSING_FIELDS：缺少必需字段
  - DUPLICATE：城市ID已存在
  - NOT_FOUND：城市不存在
  - CITY_REGISTRY_ERROR：无法读取城市注册表
...
- [admin/types/index.ts:56-67](file://admin/types/index.ts#L56-L67)
- [scripts/city-registry.json:1-233](file://scripts/city-registry.json#L1-L233)
...
- **新增**：城市数据管理
  - 管理端Cities页面通过api.get('/cities')获取城市列表
  - 通过api.post('/cities')添加新城市，api.delete('/cities/:id')删除城市
...
- 当前API未显式标注版本号（如 /api/v1/），但具备良好的向后兼容策略：
  ...
  - **新增**：城市数据管理功能采用独立的管理端路由（/api/admin/...），不影响主API的向后兼容性。
- 建议
...
I["admin/pages/Cities.tsx"] --> H
J["server/admin-routes.ts"] --> K["scripts/city-registry.json"]
```
...
- 数据体积
  ...
  - **新增**：城市注册表数据量相对较小，适合一次性加载。
- 并发与幂等
  - 酒店/POI刷新使用Set去重，避免并发重复任务。
  - **新增**：城市数据操作采用原子性写入，避免并发冲突。
- 前端优化
  - 使用本地存储缓存常用城市数据，减少网络请求。
  - 对评论与微游记采用懒加载与分页策略。
  - **新增**：城市列表支持实时搜索过滤，提升用户体验。
...
- 常见错误码
  ...
  - **新增**：城市数据管理：MISSING_FIELDS、DUPLICATE、NOT_FOUND、CITY_REGISTRY_ERROR
...
- 排查步骤
  ...
  - **新增**：对于城市数据操作，检查请求体格式是否正确，特别是经纬度范围验证。
- 日志与监控
  - 服务端会在刷新失败时输出错误日志，便于定位问题。
  - **新增**：城市数据操作的日志记录，便于追踪数据变更历史。
...
## 结论
本API体系以Express+SQLite为基础，围绕POI推荐、旅行规划、用户管理、地图服务、酒店预订与游记生态构建了完整的前端可用接口。**新增**的城市数据管理功能为移动应用提供了动态城市数据支持，通过独立的管理端路由实现，不影响主API的向后兼容性。通过三层缓存与后台刷新机制，兼顾性能与数据新鲜度；通过明确的错误码与向后兼容策略，降低前端集成成本。建议在生产环境中结合缓存与轮询策略，持续优化用户体验与稳定性。
...
- 数据模型速览
  ...
  - **新增**：城市/City：id、name、nameEn、continent、country、province、lat、lng、poiCount、lastUpdated。
- 前端工具函数
  - aiRecommend：封装POI加载、强制刷新、轮询刷新、类型转换与兼容处理。
  - routePlanner：提供智能行程生成算法，支持餐食插入、自动填充与时间窗约束。
  - **新增**：Cities页面：封装城市列表获取、搜索过滤、添加删除城市等管理功能。
