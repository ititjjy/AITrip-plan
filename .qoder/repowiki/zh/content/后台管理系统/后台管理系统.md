...
**更新** 新增采集情况仪表盘与详情页面，提供采集任务监控、城市采集统计与采集详情分析功能。新增待确认更新管理功能，提供完整的城市维度对比、批量确认/拒绝、POI 差异对比等管理能力。集成 shadcn/ui 组件库，提升 UI 一致性与开发效率。

## 项目结构
- 前端（admin）
  - 路由与布局：App、AdminLayout、Sidebar、Header
  - 页面：Dashboard、POIBrowser、PendingUpdates、ReviewQueue、Updates、CollectionDashboard、CollectionDetail
  - 类型定义：admin/types/index.ts
  - 工具与格式化：lib/api.ts、lib/formatters.ts、lib/utils.ts、hooks/useDebounce.ts
- 后端（server）
  - 管理 API：server/admin-routes.ts（统计、POI、城市、分类、更新、审核发布、采集监控等）
...
A_CollectionDash["CollectionDashboard 采集情况"]
A_CollectionDetail["CollectionDetail 采集详情"]
...
A_Layout --> A_Review
A_Layout --> A_Updates
A_Layout --> A_CollectionDash
A_Layout --> A_CollectionDetail
A_Dash --> A_API
A_Browse --> A_API
A_Pending --> A_API
A_Review --> A_API
A_Updates --> A_API
A_CollectionDash --> A_API
A_CollectionDetail --> A_API
A_API --> S_Routes
S_Routes --> S_Auth
```
...
- 路由与入口
  ...
  - 路由配置：admin/App.tsx 定义后台页面路由与 AdminLayout 布局，新增采集情况路由
- 布局组件
  - AdminLayout：整合 Sidebar 与 Header，提供主内容区域 Outlet
  - Sidebar：导航菜单，支持折叠切换，包含待确认更新菜单项和新增的'采集情况'菜单项
...
- 页面组件
  ...
  - CollectionDashboard：**新增** 采集情况仪表盘，展示采集任务统计、城市采集进度、采集质量分析
  - CollectionDetail：**新增** 采集详情页面，提供城市维度的采集任务详情、成功率统计、异常分析
- 类型系统
  - 统一定义 POI、城市、评分等级、审核状态、更新任务等类型
  - **新增** PendingUpdate、PendingUpdateDetail 类型定义
  - **新增** CollectionTask、CollectionStats、CollectionDetail 类型定义
- 工具与格式化
...
- [admin/pages/CollectionDashboard.tsx:1-150](file://admin/pages/CollectionDashboard.tsx#L1-L150)
- [admin/pages/CollectionDetail.tsx:1-200](file://admin/pages/CollectionDetail.tsx#L1-L200)
...
- 后端 admin-routes.ts 提供统计、POI、城市、分类、更新、审核发布、采集监控等接口
...
### 采集情况仪表盘（CollectionDashboard）**新增功能**
- 功能要点
  - 展示全局采集任务统计：总任务数、成功数、失败数、成功率
  - 城市维度采集进度：按城市展示采集完成度、平均耗时、质量评分
  - 采集质量分析：成功率趋势图、异常类型统计、Top 城市排名
  - 实时监控：采集任务状态实时更新，异常告警提醒
- 用户交互
  - 支持时间范围筛选（今日/本周/本月/自定义）
  - 城市筛选与排序
  - 点击城市进入采集详情页面
- 技术实现
  - 前端通过 /collection/stats、/collection/tasks 接口获取采集统计信息
  - 实时轮询更新采集状态
  - 集成图表组件展示趋势与分布

```mermaid
flowchart TD
Enter(["进入 采集情况"]) --> Stats["获取采集统计"]
Stats --> Cities["获取城市采集进度"]
Cities --> Quality["获取质量分析"]
Quality --> Render["渲染仪表盘"]
Render --> Filter{"筛选条件变化？"}
Filter --> |是| Update["重新获取数据"]
Filter --> |否| Monitor["开始实时监控"]
Monitor --> Alert{"出现异常？"}
Alert --> |是| Notify["显示告警"]
Alert --> |否| End(["完成"])
```
...
**图表来源**
- [admin/pages/CollectionDashboard.tsx:31-59](file://admin/pages/CollectionDashboard.tsx#L31-L59)
- [admin/pages/CollectionDashboard.tsx:78-120](file://admin/pages/CollectionDashboard.tsx#L78-L120)

**章节来源**
- [admin/pages/CollectionDashboard.tsx:1-150](file://admin/pages/CollectionDashboard.tsx#L1-L150)

### 采集详情（CollectionDetail）**新增功能**
- 功能要点
  - 城市采集任务详情：按时间顺序展示采集任务执行记录
  - 任务状态跟踪：成功、失败、进行中、重试等状态可视化
  - 采集质量详情：成功率、平均耗时、异常类型分布
  - 异常分析：失败原因统计、Top 异常类型、异常趋势分析
- 用户交互
  - 任务列表分页与筛选
  - 任务详情弹窗展示详细执行日志
  - 导出采集报告功能
- 技术实现
  - 前端通过 /collection/:cityId/tasks 接口获取城市采集详情
  - 支持任务重试与批量操作
  - 集成日志解析与异常分类
...
participant R as "admin-routes.ts"
U->>CD : 选择城市
CD->>A : GET /api/admin/collection/ : cityId/tasks
A->>R : 获取采集任务详情
R-->>A : 返回任务列表与统计
A-->>CD : 渲染任务详情
U->>CD : 查看任务详情
...
**图表来源**
- [admin/pages/CollectionDetail.tsx:124-200](file://admin/pages/CollectionDetail.tsx#L124-L200)
- [admin/pages/CollectionDetail.tsx:150-180](file://admin/pages/CollectionDetail.tsx#L150-L180)
...
- 关键类型
  ...
  - **新增** 采集任务：CollectionTask、CollectionStats、CollectionDetail
- 设计原则
  - 前后端类型对齐，避免字段不一致导致的渲染与逻辑错误
  - 评分等级与颜色映射集中管理，便于统一视觉风格
  - **新增** 采集相关类型统一管理，支持采集任务的完整生命周期
...
CollectionDetail --> CollectionTask : "包含"
CollectionDetail --> CollectionStats : "包含"
```
...
- 前端依赖
  ...
  - **新增** shadcn/ui 组件库：Button、Card、Table、Badge、Input、Select、Progress 等
  - 图标：lucide-react
...
## 结论
本后台管理系统围绕"数据审核、发布与监控"构建，通过清晰的页面分工与统一的类型系统，实现了从概览到细节的完整运营闭环。**新增的待确认更新功能**进一步完善了数据管理流程，提供了城市维度的对比分析和批量操作能力。**新增的采集情况模块**为采集任务监控提供了完整的解决方案，包括全局统计、城市进度跟踪与异常分析。前端注重交互体验与可访问性，后端聚焦数据一致性与安全性。集成 shadcn/ui 组件库提升了 UI 一致性与开发效率。建议持续完善权限体系与审计日志，进一步提升系统的可运维性与合规性。
