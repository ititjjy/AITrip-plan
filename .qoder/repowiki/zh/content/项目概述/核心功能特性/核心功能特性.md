...
- [src/utils/routePlanner.ts:1-1252](file://src/utils/routePlanner.ts#L1-L1252)
...
- 时间窗约束：考虑开放时间、每日窗口(08:00–22:00)，避免在非营业时段安排
...
**更新** 本次更新重点反映了路线规划算法的六大主要改进，这些改进显著提升了行程生成的质量和用户体验。

#### 1. 晚餐时间偏好调整
- **改进内容**：晚餐时间偏好从固定的18:00-20:00扩展为更灵活的时间窗口，支持根据POI类型和用户习惯进行动态调整
- **实现机制**：通过`timePrefPenalty`参数动态计算，避免在非理想时段安排晚餐活动
- **应用场景**：夜市、烧烤、居酒屋等特色晚餐场所的合理时间安排

#### 2. 日程窗口扩展
- **改进内容**：日程窗口从08:00–21:00扩展为08:00–22:00，新增对灯光秀等夜间活动的支持
- **实现机制**：`DAY_END`常量从22*60扩展，支持夜间活动的合理安排
- **技术细节**：夜间活动（如灯光秀）不惩罚晚间时间，仅在上午时才进行时间惩罚

#### 3. 地理智能餐食分配
- **改进内容**：从简单的轮询分配升级为基于地理位置的智能分配算法
- **实现机制**：`mealDistToDay`函数计算餐厅到某天路线的最短距离，考虑该天所有POI和酒店的最小距离
- **算法优势**：确保每餐都分配到地理上最合适的日期，避免跨区域用餐

#### 4. 步骤索引跟踪
- **改进内容**：新增`stepIdx`变量跟踪当前是第几个选中的POI
- **实现机制**：在贪心算法中维护步骤索引，用于购物活动的特殊惩罚逻辑
- **应用价值**：避免购物活动作为行程首个POI，优先安排景点活动

#### 5. 优先级空隙填充
- **改进内容**：从简单的空隙填充升级为基于优先级的智能填充策略
- **实现机制**：`autoFillGaps`函数识别午餐到晚餐之间的下午空档（MIN_AFTERNOON_GAP=90分钟），优先填充此类高价值时间段
- **算法优化**：下午空档优先级最高（3），普通空档次之（2），其他空档最低（1）

#### 6. 时间预算优化
- **改进内容**：用户主动选择的POI时间预算放宽策略
- **实现机制**：时间预算放宽到85%（约11.9小时/14小时），确保用户选择的POI尽量不被丢弃
- **技术细节**：`maxDayMinutes = (DAY_END - DAY_START) * 0.85`，平衡用户需求与实际可行性

```mermaid
flowchart TD
A["输入：POI列表 + 酒店分布"] --> B["地理智能餐食分配"]
B --> C["步骤索引跟踪"]
C --> D["优先级空隙填充"]
D --> E["时间预算优化"]
E --> F["晚餐时间偏好调整"]
...
**图表来源**
- [src/utils/routePlanner.ts:215-234](file://src/utils/routePlanner.ts#L215-L234)
- [src/utils/routePlanner.ts:780-786](file://src/utils/routePlanner.ts#L780-L786)
- [src/utils/routePlanner.ts:840-865](file://src/utils/routePlanner.ts#L840-L865)
- [src/utils/routePlanner.ts:189-256](file://src/utils/routePlanner.ts#L189-L256)
- [src/utils/routePlanner.ts:560-666](file://src/utils/routePlanner.ts#L560-L666)
